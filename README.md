# gwr-numpy-demo
A minimal, runnable Geographically Weighted Regression example built in pure NumPy, showing local regression, kernel weighting, bandwidth selection, and basic model diagnostics on a small synthetic spatial dataset.
...
gwr-numpy-demo/
  ...
  src/
    gwr_demo.py
  data/
    synthetic_points.csv

# gwr-numpy-demo

A small, runnable GWR example using only NumPy.

It creates a tiny synthetic spatial dataset, fits a Geographically Weighted Regression model at each sample point, and picks a bandwidth by comparing AICc-style scores.

## What this covers
- GWR local fitting (one regression per location)
- Gaussian kernel weights
- Hat matrix construction (S)
- Effective number of parameters (ENP = 2 tr(S) - tr(SᵀS))
- AICc-style scoring for bandwidth selection
- Fitted values, residuals, and a basic R²
